...
class ClientBC():
    ...
    def exec(self, query):
        if query is not None or query == '':
            ...
            if query_type == 'SELECT':
                is_query_count = True
                if 'state' in query:
                    is_query_count = False

                if is_query_count:
                    before = datetime.datetime.now()
                    result = self._select_count(query)
                    after = datetime.datetime.now()
                else:
                    before = datetime.datetime.now()
                    result = self._select_state(query)
                    after = datetime.datetime.now()
    ...
    def _select_state(self, query):
        entity_data = self._select_data_by_entity('AREA_CODE_STATE')
        response = []
        area_code = starts_in = query.split('=')[1]
        area_code = area_code.replace(' ','').replace('\'','')
        for data in entity_data:
            if data['area_code'] == area_code:
                response.append(tuple([data['state']]))
        return response

    def _select_count(self, query):
        entity_data = self._select_data_by_entity('VOTES')
        response = 0
        ids = starts_in = query.find('=') + 1
        ids = query[starts_in:-1]
        phone_number = ids.replace('\'', '').replace(' ', '')
        for data in entity_data:
            if (
                'phone_number' in data and
